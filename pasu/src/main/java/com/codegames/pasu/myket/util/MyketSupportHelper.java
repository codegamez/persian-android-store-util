...
package com.codegames.pasu.myket.util;
...
public class MyketSupportHelper {
	 ...
	 */
	public void startSetup(final OnMyketSetupFinishedListener listener) {
	...
	public void getAppUpdateStateAsync(final CheckAppUpdateListener listener) {
		...
		(new Thread(new Runnable() {
			public void run() {
				...
				final MarketUpdate finalUpdate = update;
	 ...
	 */
	private MarketUpdate getAppUpdateState() throws MyketException {
		checkSetupDone("getAppUpdateState");
		try {
			...
			return new MarketUpdate(bundle.getBoolean(RESPONSE_APP_UPDATE_AVAILABLE),
	...
	public interface CheckAppUpdateListener {
		 ...
		 */
		void onCheckAppUpdateFinished(MyketResult result, MarketUpdate update);
	}
}
